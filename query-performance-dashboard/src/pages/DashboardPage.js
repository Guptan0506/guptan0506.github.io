...
const App = () => {
    ...
    const [indexSuggestions, setIndexSuggestions] = useState('No suggestions yet. Enter query logs and click "Analyze" or "Get AI Suggestions".');
    ...
    const parseQueryLogs = () => {
        ...
        logs.forEach((line, index) => {
            ...
            if (currentQuery && currentExecutionTime !== null) {
                parsed.push({ id: parsed.length, query: currentQuery, executionTime: currentExecutionTime });
        ...
        if (parsed.length === 0) {
            logs.forEach((line, index) => {
                const combinedMatch = line.match(/(SELECT|INSERT INTO|UPDATE|DELETE FROM)\s+.*?(?:WHERE\s+.*?)?(?:ORDER BY\s+.*?)?(?:LIMIT\s+.*?)?;\s*EXECUTION_TIME:\s*(\d+(\.\d+)?)\s*ms/i);
                if (combinedMatch) {
    ...
    const analyzePerformance = () => {
        if (parsedQueries.length === 0) {
            ...
            setError('No valid queries found to analyze. Please ensure logs contain "EXECUTION_TIME: X ms" and a SQL query.');
            return;
        }
    ...
    const extractIndexCandidates = (queries) => {
        ...
        queries.forEach(q => {
            ...
            const fromMatch = q.query.match(/(?:FROM|JOIN|INSERT INTO|UPDATE|DELETE FROM)\s+([`"]?[\w\.]+[`"]?)/i);
            ...
            if (whereMatch && whereMatch[1]) {
                const conditions = whereMatch[1].split(/(AND|OR)/i).map(s => s.trim()).filter(s => !['AND', 'OR'].includes(s.toUpperCase()));
     ...
     */
    const generateBasicIndexSuggestions = () => {
        if (parsedQueries.length === 0) {
        ...
        const suggestions = Object.entries(candidateColumns)
            ...
            .map(([column, freq]) => `Consider an index on column '${column}' (used ${freq} times in WHERE/ORDER BY clauses).`);

        if (suggestions.length > 0) {
            setIndexSuggestions(suggestions.join('\n'));
        } else {
            setIndexSuggestions('No specific index candidates identified from current logs for basic suggestions.');
        }
    };
    ...
    const getAIIndexSuggestions = async () => {
        if (parsedQueries.length === 0) {
            setError('Please provide query logs and click "Analyze" first before requesting AI suggestions.');
            return;
        }
        ...
        try {
            ...
            ${parsedQueries.map(q => `- ${q.query} (Execution Time: ${q.executionTime > 0 ? q.executionTime + 'ms' : 'N/A'})`).join('\n')}

            Frequently used columns in WHERE/ORDER BY clauses:
            ${Object.entries(candidateColumns).map(([col, freq]) => `- ${col} (frequency: ${freq})`).join('\n')}
            `;
            ...
            if (result.candidates && result.candidates.length > 0 &&
                ...
                setIndexSuggestions('AI could not generate suggestions. Please try again with more detailed logs.');
            }
        } catch (err) {
            console.error("Error fetching AI suggestions:", err);
            setError(`Failed to get AI suggestions: ${err.message}. Please check your input and try again.`);
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 font-sans flex flex-col items-center">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-4xl mb-6">
                <h1 className="text-3xl font-bold text-gray-800 mb-4 text-center">Query Performance Dashboard</h1>
                <p className="text-gray-600 mb-6 text-center">
                    Enter your SQL query logs below. Each log line should contain a query and its execution time (e.g., "SELECT * FROM users WHERE id = 1; EXECUTION_TIME: 15.23 ms").
                    The parser is now more robust to handle variations.
                </p>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-4xl mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                            ...
                            <p className="text-gray-700">Total Queries Analyzed: <span className="font-bold">{performanceMetrics.totalQueries}</span></p>
                            <p className="text-gray-700">Average Execution Time: <span className="font-bold">{performanceMetrics.averageExecutionTime} ms</span></p>
                        </div>
